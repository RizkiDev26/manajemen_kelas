...
class Users extends BaseController
{
    ...
    public function index()
    {
        ...
        // Build query with filters - JOIN with tb_siswa for student data
        $db = \Config\Database::connect();
        $builder = $db->table('users u');
        $builder->select('u.*, w.kelas, ts.nama as nama_siswa, ts.nisn');
        $builder->join('walikelas w', 'u.walikelas_id = w.id', 'left');
        $builder->join('tb_siswa ts', 'u.username = ts.nisn AND u.role = "siswa"', 'left');

        // Apply search filter - include siswa name from tb_siswa
        if ($search) {
            $builder->groupStart();
            $builder->like('u.nama', $search);
            $builder->orLike('u.username', $search);
            $builder->orLike('u.email', $search);
            $builder->orLike('w.kelas', $search);
            $builder->orLike('ts.nama', $search); // Search in tb_siswa name for students
            $builder->orLike('ts.nisn', $search); // Search NISN
            $builder->groupEnd();
        }
        ...
        if ($statusFilter && $statusFilter !== 'semua') {
            $statusValue = ($statusFilter === 'aktif') ? 1 : 0;
            $builder->where('u.is_active', $statusValue);
        }
        ...
        // Post-process users data to use actual student names
        foreach ($users as &$user) {
            if ($user['role'] === 'siswa' && !empty($user['nama_siswa'])) {
                $user['nama'] = $user['nama_siswa']; // Use actual student name from tb_siswa
            }
        }

        // Get separate stats for all data (without pagination)
        $statsBuilder = $db->table('users u');
        $statsBuilder->select('u.*, w.kelas, ts.nama as nama_siswa');
        $statsBuilder->join('walikelas w', 'u.walikelas_id = w.id', 'left');
        $statsBuilder->join('tb_siswa ts', 'u.username = ts.nisn AND u.role = "siswa"', 'left');
        
        // Apply same filters for stats
        if ($search) {
            $statsBuilder->groupStart();
            $statsBuilder->like('u.nama', $search);
            $statsBuilder->orLike('u.username', $search);
            $statsBuilder->orLike('u.email', $search);
            $statsBuilder->orLike('w.kelas', $search);
            $statsBuilder->orLike('ts.nama', $search);
            $statsBuilder->orLike('ts.nisn', $search);
            $statsBuilder->groupEnd();
        }
        if ($roleFilter && $roleFilter !== 'semua') {
            $statsBuilder->where('u.role', $roleFilter);
        }
        if ($statusFilter && $statusFilter !== 'semua') {
            $statusValue = ($statusFilter === 'aktif') ? 1 : 0;
            $statsBuilder->where('u.is_active', $statusValue);
        }
        
        $allUsers = $statsBuilder->get()->getResultArray();
        
        // Calculate stats for all filtered data
        $totalAllUsers = count($allUsers);
        $totalActiveUsers = count(array_filter($allUsers, fn($u) => $u['is_active'] == 1));
        $totalAdminUsers = count(array_filter($allUsers, fn($u) => $u['role'] === 'admin'));
        $totalGuruUsers = count(array_filter($allUsers, fn($u) => in_array($u['role'], ['guru', 'walikelas'])));

        // Setup pagination
        $pager = \Config\Services::pager();
        $pagination = $pager->makeLinks($page, $perPage, $totalUsers, 'default_full');
    ...
    public function resetPassword($id)
    {
        ...
        // Default password berdasarkan role user
        $defaultPassword = 'guru123456'; // Default untuk admin dan walikelas
        if ($user['role'] === 'siswa') {
        ...
        try {
            if ($this->userModel->update($id, $updateData)) {
                $passwordInfo = ($user['role'] === 'siswa') ? 'siswa123' : 'guru123456';
                return redirect()->to('/admin/users')->with('success', 'Password user "' . $user['nama'] . '" berhasil direset ke password default: ' . $passwordInfo);
            } else {
