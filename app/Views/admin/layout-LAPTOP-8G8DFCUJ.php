...
<head>
    ...
    <style>
        :root {
            ...
            --content-padding-expanded: 4px;
            --content-padding-collapsed: 4px;
        }
        ...
        *, *::before, *::after {
        ...
        .sidebar {
            ...
            position: fixed !important; top: 0 !important; left: 0 !important; height: 100vh !important; z-index: 1000 !important;
        ...
        .content-wrapper {
            ...
            height: 100vh; padding-left: var(--content-padding-expanded);
        }
        .content-wrapper.sidebar-collapsed { margin-left: var(--sidebar-width-collapsed); padding-left: var(--content-padding-collapsed); }
        
        .fixed-header {
            position: fixed; top: 0; left: var(--sidebar-width-expanded); right: 0; z-index: 999;
        ...
        .content-area {
            flex: 1;
            overflow: visible;
            padding: 1.5rem 2rem; margin-top: 60px;
        ...
        @media (min-width: 1024px) {
            .content-area { margin-left: 0; padding: 1.5rem 2rem; max-width: none; overflow: visible !important; height: auto !important; }
            .content-wrapper:not(.sidebar-collapsed) { margin-left: var(--sidebar-width-expanded) !important; padding-left: var(--content-padding-expanded) !important; }
            .content-wrapper.sidebar-collapsed { margin-left: var(--sidebar-width-collapsed) !important; padding-left: var(--content-padding-collapsed) !important; }
        ...
        .sidebar::before {
            ...
            right: 0;
            bottom: 0;
            background: linear-gradient(180deg, rgba(255, 255, 255, 0.08) 0%, transparent 50%, rgba(0, 0, 0, 0.05) 100%);
            pointer-events: none;
        }
        ...
        .menu-item-with-submenu .submenu-toggle:hover .submenu-chevron {
            transform: rotate(180deg);
            color: rgba(255, 255, 255, 0.9);
        }
        
        .menu-item-with-submenu.active .submenu-chevron {
            transform: rotate(180deg);
        }
        ...
        .sidebar a.bg-white\/20,
        .sidebar div.bg-white\/20 {
        ...
        .submenu a.bg-white\/15::before {
            height: 60%;
        }
        
        /* Improve Logo Section */
        ...
        @keyframes pulse-glow {
            0%, 100% { 
                box-shadow: 0 0 5px rgba(52, 211, 153, 0.6), 0 0 10px rgba(52, 211, 153, 0.4); 
            }
            50% { 
                box-shadow: 0 0 10px rgba(52, 211, 153, 0.8), 0 0 20px rgba(52, 211, 153, 0.6); 
            }
        }
        ...
        /* Flash Notifications Fix - Ensure they appear above fixed header */
        .flash-notification {
            position: relative;
            z-index: 1002 !important; /* Higher than fixed-header (999) and sidebar (1000) */
            margin-top: 4rem; /* Add top margin to account for fixed header */
        }
        
        /* Animation for flash notifications */
...
<body class="main-container bg-gray-50">
    ...
    <aside class="sidebar gradient-bg shadow-xl flex flex-col relative">
        <!-- Logo Section -->
        <div class="p-6 border-b border-white/20 relative bg-gradient-to-r from-white/8 to-transparent backdrop-blur-lg">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    <div class="w-14 h-14 bg-white/15 rounded-2xl flex items-center justify-center backdrop-blur-md border border-white/25 shadow-2xl ring-1 ring-white/10">
                        <i class="fas fa-graduation-cap text-white text-2xl drop-shadow-lg"></i>
                    </div>
                    <div class="sidebar-text">
                        <h1 class="text-white text-xl font-bold tracking-tight leading-tight">SDN GU 09</h1>
                        <p class="text-white/85 text-sm font-medium leading-tight mt-0.5">Aplikasi Pengelolaan Sekolah</p>
                        <div class="flex items-center mt-2">
                            <div class="w-2 h-2 bg-emerald-400 rounded-full mr-2 shadow-lg shadow-emerald-400/50 animate-pulse"></div>
                            <span class="text-white/70 text-xs font-medium">Online</span>
                        </div>
                    </div>
                </div>
                <!-- Sidebar Toggle Button -->
                <button id="sidebarCollapse" class="sidebar-toggle-btn hidden lg:flex w-10 h-10 bg-white/10 rounded-xl items-center justify-center hover:bg-white/20 transition-all duration-300 hover:scale-105 border border-white/20 hover:shadow-lg active:scale-95" title="Tutup/Buka Sidebar">
                    <i class="fas fa-chevron-left text-white text-sm transition-transform duration-200"></i>
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        // Desktop sidebar toggle
            ...
            if (currentPath.includes('/admin/absensi/') || currentPath.includes('/absensi/')) {
                const absensiMenu = document.querySelector('.menu-item-with-submenu');
                if (absensiMenu) {
                    absensiMenu.classList.add('active');
                }
            }
            
            if (currentPath.includes('/admin/nilai/') || currentPath.includes('/nilai/')) {
                const nilaiMenus = document.querySelectorAll('.menu-item-with-submenu');
                nilaiMenus.forEach(menu => {
                    const nilaiSubmenu = menu.querySelector('a[href*="/nilai/"]');
                    if (nilaiSubmenu) {
            ...
            if (currentPath.startsWith('/guru/') || currentPath === '/siswa' || currentPath.startsWith('/siswa/')) {
                document.querySelectorAll('.menu-item-with-submenu').forEach(menu => {
                    const kebiasaanLink = menu.querySelector('a[href^="/guru/"], a[href^="/siswa"]');
                    if (kebiasaanLink) {
                        menu.classList.add('active');
                    }
                });
