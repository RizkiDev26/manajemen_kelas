...
<?php if (session()->getFlashdata('success')): ?>
    <div class="flash-notification bg-gradient-to-r from-green-50 to-emerald-50 border-l-4 border-green-400 p-4 mb-6 rounded-r-xl shadow-sm animate-slideInDown relative z-[1001]" id="successAlert">
        <div class="flex">
...
<?php if (session()->getFlashdata('error')): ?>
    <div class="flash-notification bg-gradient-to-r from-red-50 to-rose-50 border-l-4 border-red-400 p-4 mb-6 rounded-r-xl shadow-sm animate-slideInDown relative z-[1001]" id="errorAlert">
        <div class="flex">
            <div class="flex-shrink-0">
...
<div class="bg-white rounded-lg sm:rounded-xl shadow-lg border border-gray-100 p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6 lg:mb-8">
    <div class="flex flex-col gap-3 sm:gap-4 lg:gap-6">
        ...
        <div id="filterContent" class="hidden sm:block">
            <form method="GET" action="<?= base_url('admin/users') ?>" id="filterForm">
                <div class="flex flex-col sm:flex-row lg:flex-row gap-3 sm:gap-4 lg:gap-6">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="relative">
                            ...
                            <button 
                                type="button"
                                onclick="clearSearch()"
                                class="absolute right-2 sm:right-3 top-2 sm:top-3 p-1 text-gray-400 hover:text-gray-600 transition-colors duration-200 <?= empty($search) ? 'hidden' : '' ?>"
                                id="clearSearchBtn">
                    ...
                    <div class="sm:w-40 lg:w-48">
                        ...
                        <div class="relative">
                            <select 
                                ...
                                <option value="semua" <?= ($roleFilter ?? '') === 'semua' ? 'selected' : '' ?>>Semua</option>
                    ...
                    <div class="sm:w-40 lg:w-48">
                        ...
                        <div class="relative">
                            <select 
                                ...
                                <option value="nonaktif" <?= ($statusFilter ?? '') === 'nonaktif' ? 'selected' : '' ?>>Tidak Aktif</option>
                            </select>
                            <div class="absolute left-2 sm:left-3 top-2 sm:top-3.5 text-gray-400">
                    ...
                    <div class="flex gap-2">
                        <button 
                            type="submit"
                            class="px-4 py-2 sm:py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 focus:ring-2 focus:ring-purple-500 transition-all duration-200 text-sm sm:text-base font-medium">
                            <i class="fas fa-search mr-2"></i>
                            <span class="hidden sm:inline">Cari</span>
                        </button>
                        <a 
                            href="<?= base_url('admin/users') ?>"
                            class="px-4 py-2 sm:py-3 bg-gray-500 text-white rounded-lg hover:bg-gray-600 focus:ring-2 focus:ring-gray-500 transition-all duration-200 text-sm sm:text-base font-medium">
        ...
        <div class="mt-3 sm:mt-4 lg:mt-6 pt-3 sm:pt-4 lg:pt-6 border-t border-gray-200">
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 lg:gap-4">
                <div class="text-center p-2 sm:p-3 lg:p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg">
                    <div class="text-lg sm:text-xl lg:text-2xl font-bold text-blue-600" id="totalUsers"><?= $totalAllUsers ?? 0 ?></div>
                ...
                <div class="text-center p-2 sm:p-3 lg:p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg">
                    <div class="text-lg sm:text-xl lg:text-2xl font-bold text-green-600" id="activeUsers"><?= $totalActiveUsers ?? 0 ?></div>
                ...
                <div class="text-center p-2 sm:p-3 lg:p-4 bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg">
                    <div class="text-lg sm:text-xl lg:text-2xl font-bold text-purple-600" id="adminUsers"><?= $totalAdminUsers ?? 0 ?></div>
                ...
                <div class="text-center p-2 sm:p-3 lg:p-4 bg-gradient-to-br from-orange-50 to-red-50 rounded-lg">
                    <div class="text-lg sm:text-xl lg:text-2xl font-bold text-orange-600" id="teacherUsers"><?= $totalGuruUsers ?? 0 ?></div>
...
<div id="usersContainer">
                ...
                <div class="user-card bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1" 
                    ...
                    <div class="p-6 pt-4">
                        ...
                        <div class="flex space-x-2">
                            ...
                            <button 
                                onclick="resetUserPassword(<?= $user['id'] ?? 0 ?>, '<?= esc($user['nama'] ?? 'Unknown User') ?>', '<?= esc($user['role'] ?? 'admin') ?>')"
    ...
    <!-- Pagination -->
    <?php if (!empty($users) && $totalUsers > $perPage): ?>
        <div class="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white rounded-xl p-4 shadow-sm border border-gray-200">
            <!-- Results Info -->
            <div class="text-sm text-gray-600">
                Menampilkan 
                <span class="font-medium text-gray-900"><?= ($currentPage - 1) * $perPage + 1 ?></span> 
                sampai 
                <span class="font-medium text-gray-900"><?= min($currentPage * $perPage, $totalUsers) ?></span> 
                dari 
                <span class="font-medium text-gray-900"><?= $totalUsers ?></span> 
                user
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                if (!empty($roleFilter) && $roleFilter !== 'semua') $queryParams['role_filter'] = $roleFilter;
                if (!empty($statusFilter) && $statusFilter !== 'semua') $queryParams['status_filter'] = $statusFilter;
                $queryString = !empty($queryParams) ? '&' . http_build_query($queryParams) : '';
                ?>
                
                <!-- Previous Button -->
                <?php if ($currentPage > 1): ?>
                    <a href="<?= base_url('admin/users?page=' . ($currentPage - 1) . $queryString) ?>" 
                       class="flex items-center justify-center w-8 h-8 text-gray-500 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-700 transition-all duration-200">
                        <i class="fas fa-chevron-left text-xs"></i>
                    </a>
                <?php else: ?>
                    <span class="flex items-center justify-center w-8 h-8 text-gray-300 bg-gray-100 border border-gray-200 rounded-lg cursor-not-allowed">
                        <i class="fas fa-chevron-left text-xs"></i>
                    </span>
                <?php endif; ?>
                ...
                <!-- First Page -->
                <?php if ($startPage > 1): ?>
                    <a href="<?= base_url('admin/users?page=1' . $queryString) ?>" 
                       class="flex items-center justify-center w-8 h-8 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:text-purple-600 hover:border-purple-300 transition-all duration-200">
                        1
                    </a>
                    <?php if ($startPage > 2): ?>
                ...
                <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
                    <?php if ($i == $currentPage): ?>
                        <span class="flex items-center justify-center w-8 h-8 text-white bg-purple-600 border border-purple-600 rounded-lg font-medium">
                            <?= $i ?>
                        </span>
                    <?php else: ?>
                        <a href="<?= base_url('admin/users?page=' . $i . $queryString) ?>" 
                           class="flex items-center justify-center w-8 h-8 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:text-purple-600 hover:border-purple-300 transition-all duration-200">
                            <?= $i ?>
                        </a>
                    <?php endif; ?>
                ...
                <?php if ($endPage < $totalPages): ?>
                    <?php if ($endPage < $totalPages - 1): ?>
                        <span class="flex items-center justify-center w-8 h-8 text-gray-400">...</span>
                    <?php endif; ?>
                    <a href="<?= base_url('admin/users?page=' . $totalPages . $queryString) ?>" 
                       class="flex items-center justify-center w-8 h-8 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-purple-50 hover:text-purple-600 hover:border-purple-300 transition-all duration-200">
                        <?= $totalPages ?>
                    </a>
                <?php endif; ?>
...
function updateStats() {
    // Stats are now handled server-side, no need for client-side calculation
...
function resetUserPassword(userId, userName, userRole) {
    // Tentukan password default berdasarkan role
    const defaultPassword = (userRole === 'siswa') ? 'siswa123' : 'guru123456';
    
    if (confirm(`Apakah Anda yakin ingin mereset password user "${userName}" ke password default (${defaultPassword})?`)) {
