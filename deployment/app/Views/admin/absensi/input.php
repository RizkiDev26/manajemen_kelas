...
document.addEventListener('DOMContentLoaded', function() {
    // AJAX Previous/Next day navigation
    ...
    function loadAttendanceData(date) {
        ...
        if (!kelas) {
            ...
            console.log('No kelas selected');
            return;
        }
        
        console.log('Loading data for:', {date, kelas});
        ...
        .then(data => {
            ...
            if (data.success && data.data) {
                // Update students grid with new data
                updateStudentsGrid(data.data);
                
                // Update header date
                updateHeaderDate(date, data.data.selectedKelas);
                
                showToast('Data berhasil dimuat', 'success');
            } else {
                console.log('No data or failed response:', data);
                showToast(data.message || 'Tidak ada data untuk tanggal ini', 'warning');
            }
    ...
    // Function to update students grid with new data
    function updateStudentsGrid(data) {
        const studentsGrid = document.querySelector('.students-grid');
        if (!studentsGrid || !data.students) return;
            ...
            html += `
                <div class="student-card" data-siswa-id="${student.siswa_id}">
                    <div class="student-info">
                        <h5 class="student-name">${student.nama}</h5>
                        <p class="student-nipd">${student.nipd}</p>
                    </div>
                    <div class="attendance-buttons">
                        <button type="button" class="btn-attendance btn-hadir ${student.status === 'hadir' ? 'active' : ''}" data-status="hadir">
                            Hadir
                        </button>
                        <button type="button" class="btn-attendance btn-izin ${student.status === 'izin' ? 'active' : ''}" data-status="izin">
                            Izin
                        </button>
                        <button type="button" class="btn-attendance btn-sakit ${student.status === 'sakit' ? 'active' : ''}" data-status="sakit">
                            Sakit
                        </button>
                    </div>
                    <input type="hidden" class="student-keterangan" value="${student.keterangan || ''}">
    ...
    // Function to update header date
    function updateHeaderDate(date, kelas) {
        const dayNames = {
            'Sunday': 'Minggu',
            'Monday': 'Senin',
            'Tuesday': 'Selasa', 
        ...
        const indonesianDay = dayNames[englishDay] || englishDay;
        ...
        console.log('Updating header:', {date, indonesianDay, formattedDate, kelas});
        
        const headerTitle = document.querySelector('.header-title');
        if (headerTitle) {
            headerTitle.innerHTML = `
                <h2 class="mb-0">Absensi Kelas ${kelas}</h2>
                <p class="mb-0 date-display">${indonesianDay}, ${formattedDate}</p>
            `;
    ...
    document.getElementById('tanggal')?.addEventListener('change', function() {
        ...
        if (kelas && newDate) {
            ...
            if (studentsGrid) {
                ...
                loadingDiv.innerHTML = '<div style="text-align: center; color: #667eea; font-weight: bold;"><i class="fas fa-spinner"></i> Memuat data tanggal ' + newDate + '...</div>';
                studentsGrid.appendChild(loadingDiv);
            }
            
            // Redirect to new URL - this will reload the page with new data
            setTimeout(() => {
    ...
    document.getElementById('kelas')?.addEventListener('change', function() {
        ...
        if (newKelas && date) {
            ...
            if (studentsGrid) {
                ...
                const loadingDiv = document.createElement('div');
                loadingDiv.className = 'loading-overlay';
                loadingDiv.innerHTML = '<div style="text-align: center; color: #667eea; font-weight: bold;"><i class="fas fa-spinner"></i> Memuat data kelas ' + newKelas + '...</div>';
                studentsGrid.appendChild(loadingDiv);
            }
