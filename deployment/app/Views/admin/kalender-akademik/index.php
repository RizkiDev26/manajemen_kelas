        ...
        <div class="p-6">
            ...
            <form id="dateSettingForm">
                ...
                <div class="mt-6 flex space-x-3">
                    <button type="button" onclick="closeDateModal()" class="flex-1 bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg font-medium transition-colors duration-200">
                        Batal
                    </button>
                    <button type="button" onclick="deleteStatus()" class="flex-1 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-medium transition-colors duration-200">
                        Hapus Status
                    </button>
...
document.getElementById('dateSettingForm').addEventListener('submit', function(e) {
    ...
    fetch('/admin/kalender-akademik/save-event', {
        method: 'POST',
        body: formData,
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
...
function deleteStatus() {
    ...
    if (!confirm('Apakah Anda yakin ingin menghapus semua status untuk tanggal ini?')) {
        return;
    }
